...
// Enums
// ========================
export const orderTypeEnum = pgEnum('order_type', ['ticket', 'visa', 'reschedule', 'cancellation'])
...
export const accountTypeEnum = pgEnum('account_type', ['cash', 'bank', 'customer', 'vendor'])
...
export const rolePermissionsModel = pgTable('role_permissions', {
  roleId: integer('role_id').references(() => roleModel.roleId),
  permissionId: integer('permission_id').notNull().references(() => permissionsModel.id),
})

export const userRolesModel = pgTable('user_roles', {
  userId: integer('user_id').notNull().references(() => userModel.userId),
  roleId: integer('role_id').notNull().references(() => roleModel.roleId),
})
...
export const orderServiceModel = pgTable('order_services', {
  ...
  procurementStatus: varchar('procurement_status', { length: 30 }).default('pending'),
})
...
export const installmentModel = pgTable('installments', {
  id: uuid('id').defaultRandom().primaryKey(),
  packageCustomerId: uuid('package_customer_id').references(() => packageCustomerModel.id),
